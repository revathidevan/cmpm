...
def modify_excel_file():
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            # Add thick borders to header row (9th row)
            for col in range(1, len(modified_header) + 1):
                col_letter = openpyxl.utils.get_column_letter(col)
                current_cell = worksheet[f'{col_letter}9']
                
                # First column of header row
                if col == 1:
                    current_cell.border = openpyxl.styles.Border(
                        left=openpyxl.styles.Side(style='thick'),
                        right=openpyxl.styles.Side(style='thin'),
                        top=openpyxl.styles.Side(style='thick'),
                        bottom=openpyxl.styles.Side(style='thick')
                    )
                # Last column of header row
                elif col == len(modified_header):
                    current_cell.border = openpyxl.styles.Border(
                        left=openpyxl.styles.Side(style='thin'),
            ...
            # Add thick borders to subtotal row
            for col in range(1, len(modified_header) + 1):
                col_letter = openpyxl.utils.get_column_letter(col)
                current_cell = worksheet[f'{col_letter}{subtotal_row}']
                
                # First column of subtotal row
                if col == 1:
                    current_cell.border = openpyxl.styles.Border(
                        left=openpyxl.styles.Side(style='thick'),
                        right=openpyxl.styles.Side(style='thin'),
                ...
                # Middle columns of subtotal row
                else:
                    current_cell.border = openpyxl.styles.Border(
                        left=openpyxl.styles.Side(style='thin'),
                        right=openpyxl.styles.Side(style='thin'),
